...
const (
	// lookupSubject is the NATS subject for v1-sync-helper lookup
...
func (m *NATSMapper) MapCommitteeV2ToV1(ctx context.Context, v2UID string) (string, error) {
	...
	return parseCommitteeResponse(response)
}

// parseCommitteeResponse parses the v1-sync-helper response for committee lookups.
// Responses may be plain SFIDs or compound "projectSFID:committeeSFID" — only the
// committee SFID is returned.
func parseCommitteeResponse(response string) (string, error) {
